...
ADD https://cspkg.s3.amazonaws.com/cmapi/pr/220/mariadb-columnstore-cmapi.tar.gz /opt/cmapi
WORKDIR /opt/cmapi
RUN tar -xvzf mariadb-columnstore-cmapi.tar.gz && \
    rm -f mariadb-columnstore-cmapi.tar.gz && \
    rm -rf /opt/cmapi/service* && \
    /usr/bin/cp -f /opt/cmapi/engine_files/mcs-loadbrm.py /usr/bin/mcs-loadbrm.py
WORKDIR /
